...
{
    public partial class frmClasseTerapeutica : Form
    {
        private readonly EPharmacyContext _context;
        private int Id;


        public frmClasseTerapeutica()
        {
            InitializeComponent();
        ...
        {
            DialogResult resultado = MessageBox.Show(
                "Você tem certeza que deseja Excluir a Classe Terapeutica Selecionado?",
                "Confirmação",
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Question
            );
            ...
            var Delete = _context.ClasseTerapeutica.Find(Id_);

            if (Delete != null)
            {
                var medicamento = _context.Medicamento.FirstOrDefault(b => b.ClasseTerapeuticaId == Id_);

                if (medicamento != null)
                {
                    MessageBox.Show("Classe Terapeutica não pode ser excluído. Tem dados relacionados entre Medicamento com Classe Terapeutica!");
                    return;
                }

                _context.ClasseTerapeutica.Remove(Delete);
                _context.SaveChangesAsync();
                MessageBox.Show("Classe Terapeutica excluído com sucesso!");
            ...
            {
                MessageBox.Show("Classe Terapeutica não encontrado.");
            }
        }
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            var entityNew = new ClasseTerapeutica();
            var entityUpdate = new ClasseTerapeutica();

            if (txtId.Text.IsNullOrEmpty())
            {
                entityNew = new ClasseTerapeutica
                {
                    Descricao = Descricao_,
                };
                _context.ClasseTerapeutica.Add(entityNew);
            ...
            {
                int Id_ = Convert.ToInt32(txtId.Text);
                entityUpdate = _context.ClasseTerapeutica.Find(Id_);
            ...
            MessageBox.Show(
                "Classe Terapeutica Gravado com sucesso",
                "",
        ...
        private void btnPesquisar_Click(object sender, EventArgs e)
        {
            ...
            var entidade = _context.ClasseTerapeutica.AsQueryable();
